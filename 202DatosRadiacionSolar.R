...
fincas.geo <- fincas.geo %>% 
  select(FINCA, FECHA_SIEMBRA, FECHA_COSECHA, RENDIMIENTO) %>% 
...
for(i in 1:length(fechas)){
    ...
    for(j in 1:nrow(fincas.fecha)){
    
        distancias <- st_distance(fincas.fecha[j,], rad.fecha)
        valor <- rad.fecha[which.min(distancias),]$Valor
        
        info.temp <- fincas.fecha[j,] %>% 
          bind_cols(data.frame("Variable"=c("RADIACION"),
                               "Valor"=valor)) %>% 
          st_drop_geometry()
